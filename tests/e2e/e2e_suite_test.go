...
// runInitialNode builds a docker image capable of running an Guru node with the given version.
...
func (s *IntegrationTestSuite) runInitialNode(version upgrade.VersionConfig) {
	...
	s.Require().NoError(err, "can't build container with Guru version: %s", version.ImageTag)
	...
	s.Require().NoError(err, "can't run node with Guru version: %s", version)
...
// runNodeWithCurrentChanges builds a docker image using the current branch of the Guru repository.
...
func (s *IntegrationTestSuite) proposeUpgrade(name, target string) {
	...
	// if Guru is lower than v10.x.x no need to use the legacy proposal
	currentVersion, err := s.upgradeManager.GetNodeVersion(ctx)
	s.Require().NoError(err, "can't get current Guru version")
	...
	s.Require().NoErrorf(
		err,
		"can't create the proposal to upgrade Guru to %s at height %d with name %s",
	...
	s.Require().NoErrorf(
		err,
		"failed to submit proposal to upgrade Guru to %s at height %d\nstdout: %s,\nstderr: %s",
...
func (s *IntegrationTestSuite) voteForProposal(id int) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	exec, err := s.upgradeManager.CreateVoteProposalExec(s.upgradeParams.ChainID, id, "--fees=500aguru", "--gas=500000")
...
func (s *IntegrationTestSuite) upgrade(targetRepo, targetVersion string) {
	...
	// export node .gurud to local build/
	...
	node.SetCmd([]string{"gurud", "start", fmt.Sprintf("--chain-id=%s", s.upgradeParams.ChainID)})
	err = s.upgradeManager.RunNode(node)
