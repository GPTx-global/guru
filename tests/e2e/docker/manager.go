...
type Manager struct {
	...
	// Nodes stores the list of active containers by branch
	Nodes map[string]*dockertest.Resource
}
...
func NewManager(networkName string) (*Manager, error) {
	...
	return &Manager{
		pool:    pool,
		network: network,
		Nodes:   make(map[string]*dockertest.Resource),
	}, nil
}
...
func (m *Manager) WaitForHeight(ctx context.Context, key string, height int) (string, error) {
	...
	for {
		select {
		case <-ticker.C:
			stdOut, stdErr, errLogs := m.GetLogs(m.ContainerID(key))
			if errLogs != nil {
		...
		default:
			currentHeight, err = m.GetNodeHeight(ctx, key)
...
func (m *Manager) GetNodeHeight(ctx context.Context, key string) (int, error) {
	exec, err := m.CreateExec([]string{"gurud", "q", "block"}, m.ContainerID(key))
	if err != nil {
...
func (m *Manager) GetNodeVersion(ctx context.Context, key string) (string, error) {
	exec, err := m.CreateExec([]string{"gurud", "version"}, m.ContainerID(key))
	if err != nil {
...
func (m *Manager) ContainerID(key string) string {
	return m.Nodes[key].Container.ID
}
...
func (m *Manager) KillNode(key string) error {
	return m.pool.Client.StopContainer(m.ContainerID(key), 5)
}

// RemoveNode removes the container
func (m *Manager) RemoveNode(key string) error {
	options := &docker.RemoveContainerOptions{
		ID:            m.ContainerID(key),
		RemoveVolumes: true,
		Force:         true,
...
func (m *Manager) RemoveAllNodes() error {
	for key := range m.Nodes {
		if err := m.RemoveNode(key); err != nil {
			return err
		}
	}
	return nil
