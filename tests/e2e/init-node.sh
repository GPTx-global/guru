...
# Set client config
gurud config keyring-backend "$KEYRING"
gurud config chain-id "$CHAINID"

# if $KEY exists it should be deleted
gurud keys add "$KEY" --keyring-backend $KEYRING --algo "$KEYALGO"

# Set moniker and chain-id for Guru (Moniker can be anything, chain-id must be an integer)
gurud init "$MONIKER" --chain-id "$CHAINID"
...
node_address=$(gurud keys list | grep  "address: " | cut -c12-)
...
gurud add-genesis-account $KEY 100000000000000000000000000aguru --keyring-backend $KEYRING
...
gurud gentx $KEY 1000000000000000000000aguru --keyring-backend $KEYRING --chain-id "$CHAINID"
...
# Collect genesis tx
gurud collect-gentxs

# Run this to ensure everything worked and that the genesis file is setup correctly
gurud validate-genesis

# Start the node (remove the --pruning=nothing flag if historical queries are not needed)
gurud start "$TRACE" --log_level $LOGLEVEL --minimum-gas-prices=0.0001aguru --json-rpc.api eth,txpool,personal,net,debug,web3
