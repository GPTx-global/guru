...
func DeployContract(
	...
	chainID := guruApp.EvmKeeper.ChainID()
	from := common.BytesToAddress(priv.PubKey().Address().Bytes())
	nonce := guruApp.EvmKeeper.GetNonce(ctx, from)
	...
	msgEthereumTx := evm.NewTx(&evm.EvmTxArgs{
		...
		Nonce:     nonce,
		GasLimit:  gas,
		GasFeeCap: guruApp.FeeMarketKeeper.GetBaseFee(ctx),
	...
	res, err := DeliverEthTx(guruApp, priv, msgEthereumTx)
	if err != nil {
		return common.Address{}, err
	}

	if _, err := CheckEthTxResponse(res, guruApp.AppCodec()); err != nil {
		return common.Address{}, err
	}
...
func DeployContractWithFactory(
	...
	chainID := guruApp.EvmKeeper.ChainID()
	from := common.BytesToAddress(priv.PubKey().Address().Bytes())
	factoryNonce := guruApp.EvmKeeper.GetNonce(ctx, factoryAddress)
	nonce := guruApp.EvmKeeper.GetNonce(ctx, from)
