...
## default port prefixes for gurud
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
    ...
    "$PWD"/build/gurud gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/gurud collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/gurud validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    "$PWD"/build/gurud start --pruning=nothing --rpc.unsafe \
    ...
    GURU_PID=$!
    echo "started guru node, pid=$GURU_PID"
    # add PID to array
    arr+=("$GURU_PID")
...
if [[ -z $TEST || $TEST == "rpc" ||  $TEST == "pending" ]]; then
    ...
    for i in $(seq 1 "$TEST_QTD"); do
        ...
        echo "going to test guru node $HOST_RPC ..."
...
stop_func() {
    GURU_PID=$i
    echo "shutting down node, pid=$GURU_PID ..."
