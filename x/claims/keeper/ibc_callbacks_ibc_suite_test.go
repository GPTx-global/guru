...
func (suite *IBCTestingSuite) SetupTest() {
	...
	evmosChainA := suite.chainA.App.(*app.Guru)
	evmosChainB := suite.chainB.App.(*app.Guru)
	...
	err = testutil.FundModuleAccount(suite.chainB.GetContext(), suite.chainB.App.(*app.Guru).BankKeeper, types.ModuleName, coins)
	suite.Require().NoError(err)

	suite.chainB.App.(*app.Guru).ClaimsKeeper.SetClaimsRecord(suite.chainB.GetContext(), addr, claimsRecord)

	err = testutil.FundModuleAccount(suite.chainA.GetContext(), suite.chainA.App.(*app.Guru).BankKeeper, types.ModuleName, coins)
	suite.Require().NoError(err)

	suite.chainA.App.(*app.Guru).ClaimsKeeper.SetClaimsRecord(suite.chainA.GetContext(), addr, claimsRecord)

	params := types.DefaultParams()
	params.AirdropStartTime = suite.chainA.GetContext().BlockTime()
	params.EnableClaims = true
	err = suite.chainA.App.(*app.Guru).ClaimsKeeper.SetParams(suite.chainA.GetContext(), params)
	suite.Require().NoError(err)
	err = suite.chainB.App.(*app.Guru).ClaimsKeeper.SetParams(suite.chainB.GetContext(), params)
	suite.Require().NoError(err)
...
func (suite *IBCTestingSuite) TestOnAcknowledgementPacketIBC() {
	sender := "guru10jmp6sgh4cc6zt3e8gw05wavvejgr5pwggsdaj"   //nolint:goconst
	receiver := "guru1cml96vmptgw99syqrrz8az79xer2pcgpawsch9" //nolint:goconst
	...
	testCases := []struct {
	...
	}{
		{
			...
			func(_ int64) {
				...
				suite.chainA.App.(*app.Guru).ClaimsKeeper.SetParams(suite.chainA.GetContext(), params) //nolint:errcheck
				suite.chainB.App.(*app.Guru).ClaimsKeeper.SetParams(suite.chainB.GetContext(), params) //nolint:errcheck
			},
			4,
			0,
			false,
		...
		},
		{
			...
			func(claimableAmount int64) {
				...
				coins := sdk.NewCoins(sdk.NewCoin("aguru", amt))

				suite.chainA.App.(*app.Guru).ClaimsKeeper.SetClaimsRecord(suite.chainA.GetContext(), senderAddr, types.NewClaimsRecord(amt))
		...
		},
		{
			...
			func(claimableAmount int64) {
				...
				suite.chainA.App.(*app.Guru).ClaimsKeeper.SetClaimsRecord(suite.chainA.GetContext(), senderAddr, types.ClaimsRecord{InitialClaimableAmount: amt, ActionsCompleted: []bool{true, true, true, true}})
			},
			4,
			0,
			true,
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			transfer := transfertypes.NewFungibleTokenPacketData("aguru", "100", sender, receiver, "")
			...
			coin := suite.chainA.App.(*app.Guru).BankKeeper.GetBalance(suite.chainA.GetContext(), senderAddr, "aguru")
			suite.Require().Equal(sdk.NewCoin("aguru", sdk.NewInt(tc.expectedBalance)).String(), coin.String())
			_, found := suite.chainA.App.(*app.Guru).ClaimsKeeper.GetClaimsRecord(suite.chainA.GetContext(), senderAddr)
			if tc.expPass {
...
func (suite *IBCTestingSuite) TestOnRecvPacketIBC() {
	sender := "guru10jmp6sgh4cc6zt3e8gw05wavvejgr5pwggsdaj"
	receiver := "guru1cml96vmptgw99syqrrz8az79xer2pcgpawsch9"
	...
	testCases := []struct {
		...
		},
		{
			...
			func(claimableAmount int64) {
				amt := sdk.NewInt(claimableAmount)
				coins := sdk.NewCoins(sdk.NewCoin("aguru", sdk.NewInt(claimableAmount/4)))

				suite.chainB.App.(*app.Guru).ClaimsKeeper.SetClaimsRecord(suite.chainB.GetContext(), senderAddr, types.ClaimsRecord{InitialClaimableAmount: amt, ActionsCompleted: []bool{false, true, true, true}})
				// update the escrowed account balance to maintain the invariant
				err := testutil.FundModuleAccount(suite.chainB.GetContext(), suite.chainB.App.(*app.Guru).BankKeeper, types.ModuleName, coins)
				suite.Require().NoError(err)
			},
			func() {
				...
				_, found := suite.chainB.App.(*app.Guru).ClaimsKeeper.GetClaimsRecord(suite.chainB.GetContext(), senderAddr)
				suite.Require().True(found)
			},
			4,
			0,
		...
		},
		{
			...
			func(claimableAmount int64) {
				...
				suite.chainB.App.(*app.Guru).ClaimsKeeper.SetClaimsRecord(suite.chainB.GetContext(), senderAddr, types.ClaimsRecord{InitialClaimableAmount: amt, ActionsCompleted: []bool{false, false, false, true}})
				suite.chainB.App.(*app.Guru).ClaimsKeeper.SetClaimsRecord(suite.chainB.GetContext(), receiverAddr, types.ClaimsRecord{InitialClaimableAmount: amt, ActionsCompleted: []bool{true, true, true, false}})
		...
		},
		{
			...
			func(claimableAmount int64) {
				amt := sdk.NewInt(claimableAmount)
				coins := sdk.NewCoins(sdk.NewCoin("aguru", amt.Add(amt.QuoRaw(2))))

				suite.chainB.App.(*app.Guru).ClaimsKeeper.SetClaimsRecord(suite.chainB.GetContext(), senderAddr, types.ClaimsRecord{InitialClaimableAmount: amt, ActionsCompleted: []bool{false, false, false, false}})
				suite.chainB.App.(*app.Guru).ClaimsKeeper.SetClaimsRecord(suite.chainB.GetContext(), receiverAddr, types.ClaimsRecord{InitialClaimableAmount: amt, ActionsCompleted: []bool{false, true, true, false}})
		...
		},
		{
			...
			func(claimableAmount int64) {
				amt := sdk.NewInt(claimableAmount)
				coins := sdk.NewCoins(sdk.NewCoin("aguru", amt.QuoRaw(2)))

				suite.chainB.App.(*app.Guru).ClaimsKeeper.SetClaimsRecord(suite.chainB.GetContext(), senderAddr, types.ClaimsRecord{InitialClaimableAmount: sdk.ZeroInt(), ActionsCompleted: []bool{false, false, false, false}})
				suite.chainB.App.(*app.Guru).ClaimsKeeper.SetClaimsRecord(suite.chainB.GetContext(), receiverAddr, types.ClaimsRecord{InitialClaimableAmount: amt, ActionsCompleted: []bool{false, true, true, false}})
		...
		},
		{
			...
			func(claimableAmount int64) {
				amt := sdk.NewInt(claimableAmount)
				coins := sdk.NewCoins(sdk.NewCoin("aguru", amt))
				suite.chainB.App.(*app.Guru).ClaimsKeeper.SetClaimsRecord(suite.chainB.GetContext(), senderAddr, types.NewClaimsRecord(amt))
		...
		},
		{
			...
			func(claimableAmount int64) {
				amt := sdk.NewInt(claimableAmount)
				suite.chainB.App.(*app.Guru).ClaimsKeeper.SetClaimsRecord(suite.chainB.GetContext(), receiverAddr, types.ClaimsRecord{InitialClaimableAmount: amt, ActionsCompleted: []bool{false, false, false, false}})
		...
		},
		{
			...
			func(claimableAmount int64) {
				amt := sdk.NewInt(claimableAmount)
				suite.chainB.App.(*app.Guru).ClaimsKeeper.SetClaimsRecord(suite.chainB.GetContext(), receiverAddr, types.ClaimsRecord{InitialClaimableAmount: amt, ActionsCompleted: []bool{true, true, true, true}})
			},
			func() {},
			4,
			0,
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			transfer := transfertypes.NewFungibleTokenPacketData("aguru", triggerAmt, sender, receiver, "")
			bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
			packet := channeltypes.NewPacket(bz, 1, path.EndpointA.ChannelConfig.PortID, path.EndpointA.ChannelID, path.EndpointB.ChannelConfig.PortID, path.EndpointB.ChannelID, timeoutHeight, 0)
			...
			coin := suite.chainB.App.(*app.Guru).BankKeeper.GetBalance(suite.chainB.GetContext(), receiverAddr, "aguru")
			suite.Require().Equal(coin.String(), sdk.NewCoin("aguru", sdk.NewInt(tc.expectedBalance)).String())
			_, found := suite.chainB.App.(*app.Guru).ClaimsKeeper.GetClaimsRecord(suite.chainB.GetContext(), receiverAddr)
