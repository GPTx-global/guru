...
var _ = Describe("Convert receiving IBC to Erc20", Ordered, func() {
	...
	evmosMeta := banktypes.Metadata{
		...
		DenomUnits: []*banktypes.DenomUnit{
			{
				Denom:    teststypes.AguruDenomtrace.BaseDenom,
				Exponent: 0,
			},
		},
		Name:    utils.BaseDenom,
		Symbol:  erc20Symbol,
		Display: teststypes.AguruDenomtrace.BaseDenom,
	}
	...
	Describe("enabled params and registered uosmo", func() {
		...
		It("should transfer and not convert aguru", func() {
			// Register 'aguru' coin in ERC-20 keeper to validate it is not converting the coins when receiving 'aguru' thru IBC
			pair, err := s.app.Erc20Keeper.RegisterCoin(s.EvmosChain.GetContext(), evmosMeta)
			s.Require().NoError(err)

			aguruInitialBalance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, utils.BaseDenom)

			// 1. Send aguru from Evmos to Osmosis
			s.SendAndReceiveMessage(s.pathOsmosisEvmos, s.EvmosChain, utils.BaseDenom, amount, receiver, sender, 1, "")

			aguruAfterBalance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, utils.BaseDenom)
			s.Require().Equal(aguruInitialBalance.Amount.Sub(math.NewInt(amount)).Sub(sendAndReceiveMsgFee), aguruAfterBalance.Amount)

			// check ibc aguru coins balance on Osmosis
			aguruIBCBalanceBefore := s.IBCOsmosisChain.GetSimApp().BankKeeper.GetBalance(s.IBCOsmosisChain.GetContext(), senderAcc, teststypes.AguruIbcdenom)
			s.Require().Equal(amount, aguruIBCBalanceBefore.Amount.Int64())

			// 2. Send aguru IBC coins from Osmosis to Evmos
			ibcCoinMeta := fmt.Sprintf("%s/%s", teststypes.AguruDenomtrace.Path, teststypes.AguruDenomtrace.BaseDenom)
			s.SendBackCoins(s.pathOsmosisEvmos, s.IBCOsmosisChain, teststypes.AguruIbcdenom, amount, sender, receiver, 1, ibcCoinMeta)

			// check ibc aguru coins balance on Osmosis - should be zero
			aguruIBCSenderFinalBalance := s.IBCOsmosisChain.GetSimApp().BankKeeper.GetBalance(s.IBCOsmosisChain.GetContext(), senderAcc, teststypes.AguruIbcdenom)
			s.Require().Equal(int64(0), aguruIBCSenderFinalBalance.Amount.Int64())

			// check aguru balance after transfer - should be equal to initial balance
			aguruFinalBalance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, utils.BaseDenom)

			totalFees := sendBackCoinsFee.Add(sendAndReceiveMsgFee)
			s.Require().Equal(aguruInitialBalance.Amount.Sub(totalFees), aguruFinalBalance.Amount)

			// check IBC Coin balance - should be zero
			ibcCoinsBalance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, teststypes.AguruIbcdenom)
	...
	Describe("Performing claims with registered coin", func() {
		...
		It("it should perform the claim and convert the received tokens", func() {
			...
			receiverInitialAguruBalance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, utils.BaseDenom)
			...
			// should trigger claims logic and send aguru coins from claims to receiver
			...
			receiverFinalAguruBalance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, utils.BaseDenom)

			s.Require().Equal(receiverInitialAguruBalance.Amount.Add(claimableAmount).Sub(sendBackCoinsFee), receiverFinalAguruBalance.Amount)
		})
	})
...
var _ = Describe("Convert outgoing ERC20 to IBC", Ordered, func() {
	...
	Describe("disabled params", func() {
		...
		It("should fail transfer and not convert to IBC", func() {
			...
			denom := originChain.App.(*app.Guru).StakingKeeper.BondDenom(originChain.GetContext())
