...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	# Set client config
	$DENOM config keyring-backend $KEYRING --home "$HOMEDIR"
	$DENOM config chain-id $CHAINID --home "$HOMEDIR"

	# If keys exist they should be deleted
	for KEY in "${KEYS[@]}"; do
		$DENOM keys add "$KEY" --keyring-backend $KEYRING --algo $KEYALGO --home "$HOMEDIR"
	done

	# Set moniker and chain-id for Guru (Moniker can be anything, chain-id must be an integer)
	$DENOM init $MONIKER -o --chain-id $CHAINID --home "$HOMEDIR"
	...
	claims_key=${KEYS[0]}
	node_address=$($DENOM keys show "$claims_key" --keyring-backend $KEYRING --home "$HOMEDIR" | grep "address" | cut -c12-)
	...
	mod_key=${KEYS[1]}
	base_key=${KEYS[1]}
	jq -r --arg moderator_address "$($DENOM keys show $mod_key --address --keyring-backend "$KEYRING" --home "$HOMEDIR")" '.app_state["distribution"]["moderator_address"] = $moderator_address' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	jq -r --arg base_address "$($DENOM keys show $base_key --address --keyring-backend "$KEYRING" --home "$HOMEDIR")" '.app_state["distribution"]["base_address"] = $base_address' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	...
	for KEY in "${KEYS[@]}"; do
		$DENOM add-genesis-account "$KEY" 100000000000000000000000000aguru --keyring-backend $KEYRING --home "$HOMEDIR"
	done
	...
	$DENOM gentx "${KEYS[0]}" 1000000000000000000000aguru --keyring-backend $KEYRING --chain-id $CHAINID --home "$HOMEDIR"
	...
	# Collect genesis tx
	$DENOM collect-gentxs --home "$HOMEDIR"

	# Run this to ensure everything worked and that the genesis file is setup correctly
	$DENOM validate-genesis --home "$HOMEDIR"
...
$DENOM start --metrics "$TRACE" --log_level $LOGLEVEL --minimum-gas-prices=0.0001aguru --json-rpc.api eth,txpool,personal,net,debug,web3 --api.enable --home "$HOMEDIR"
