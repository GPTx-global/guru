...
HOMEDIR="$HOME/.tmp-gurud-station"
# to trace evm
#TRACE="--trace"
TRACE=""
...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	VAL_KEY=${KEYS[0]}
	VAL_MNEMONIC="invite scissors burst color they bind hole exercise update brass hunt face crouch potato draft violin train number antenna witness foam hobby neutral token"
	...
	DEV1_KEY=${KEYS[1]}
	DEV1_MNEMONIC="control april tomato actress tattoo reward staff deer stock possible impulse below sketch ladder minute banana dune salute slender assault hawk favorite marble captain"
	...
	DEV2_KEY=${KEYS[2]}
	DEV2_MNEMONIC="scan culture country impact scatter wife cat find burst injury enrich bubble gasp like congress tourist control oval tail panther era amazing sugar agree"
	
	# address: guru1g2escvs829zhxlrqha2dygtqj5u0j8h23fe9s8
	HERMES_KEY=${KEYS[3]}
	HERMES_MNEMONIC="basket visual exclude affair drive ten breeze diagram real play tattoo connect enhance pupil oval comfort century museum tunnel rely couch coffee bird conduct"
	
	# address: guru16dpm4ysj6g865kpvhdey9a9qalpej306kjmwzj
	ADMIN_KEY=${KEYS[4]}
	ADMIN_MNEMONIC="script hawk level eye stick ensure tree buyer language task unfold oil chef load castle area bench arrive insect tackle rose point host pottery"
	
	# address: guru1ggul5tg2y80p9slpeqnw9nzy8k6wrnx0ekmr25
	RESERVE_KEY=${KEYS[5]}
	RESERVE_MNEMONIC="glove divert middle nice palace inner crack salute volcano example bulk blue april oval uncover guilt purse ginger safe ribbon palm garlic curious car"
	...
	echo "$HERMES_MNEMONIC" | $DNOM keys add "$HERMES_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	echo "$ADMIN_MNEMONIC" | $DNOM keys add "$ADMIN_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	echo "$RESERVE_MNEMONIC" | $DNOM keys add "$RESERVE_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	...
	# Set the cex module moderator address
	jq -r --arg moderator_address "$($DNOM keys show $mod_key --address --keyring-backend "$KEYRING" --home "$HOMEDIR")" '.app_state["cex"]["moderator_address"] = $moderator_address' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	...
	# .app_state.bank.balances += [
		...
		# 	"coins": [
		# 		{
		# 			"denom": "ibc/68A683DCEB6E23D57BDD3496DFDE0DF813D41A4B268F9609E9ED1360B6DF
		# 			"amount": "1000000000000000000000000"
		# 		},
		# 		{
		# 			"denom": "ibc/B5ECE4E9A5A906E43D14245C73A8771B9B8BCB5D202EEDE99B73C9FC59D73677",
		# 			"amount": "1000000000000000000000000"
		# 		}
		# 	]
		# }
	...
	# hashes=(
	# 	68A683DCEB6E23D57BDD3496DFDE0DF813D41A4B268F9609E9ED1360B6DF0294
	# 	B5ECE4E9A5A906E43D14245C73A8771B9B8BCB5D202EEDE99B73C9FC59D73677
	# )

	# index=1
	# for hash in "${hashes[@]}"; do
	# 	jq --arg hash_denom "ibc/$hash" --argjson index "$index" \
	# 	'.app_state["bank"]["supply"][$index]["denom"] = $hash_denom' "$GENESIS" > "$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"

	# 	jq --argjson index "$index" \
	# 	'.app_state["bank"]["supply"][$index]["amount"] = "1000000000000000000000000"' "$GENESIS" > "$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
